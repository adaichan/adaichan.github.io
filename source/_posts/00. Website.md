...
为了把本地文件上传到 GitHub，需要用到目前最先进的分布式版本控制工具 **Git**，不得不说 **Linus Torvalds** 永远滴神，在下的偶像，基于这个之后又有了全球最大的同性交友平台 GitHub，三个旧金山的年轻人在2008年4月创办，正如每个伟大的传奇都开始于一场冒险，Tom 在这篇文章 [我如何辞掉微软30万年薪邀约，创办GitHub](http://tom.preston-werner.com/2008/10/18/how-i-turned-down-300k.html) 中谈到：当我老去，回顾一生，我想说，”哇，那是一场冒险“；而不是，“哇，我真的很安稳“。
...
Git 和 Node.js 安装好后，就可以安装 Hexo 了，你可以先创建一个文件夹比如叫 Blog，用来存放自己的博客文件。

在该目录下右键空白点击 `Git Bash Here` 定位到该目录下，输入命令 `npm install -g hexo-cli` 安装 `Hexo`。

```bash
npm install -g hexo-cli
```

安装完后输入`hexo -v`验证是否安装成功，接下来初始化一下 Hexo，输入`hexo init`初始化文件夹
...
如此这般本地的网站配置完毕后，输入`hexo g`生成静态网页，然后输入`hexo s`打开本地服务器
