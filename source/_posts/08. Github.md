...
mathjax: false
password:
summary: 在 Github 上可以删除整个仓库，也可以移除某个文件，但是对于文件夹则只能通过在本地建立仓库关联云端，采取命令的形式来删除。虽然不是很方便，但偶尔也会遇到需要删除某些文件夹的场景，具体步骤参考下文。
tags:
- Github
categories:
- 工具
...
1. 进入到我们需要删除的仓库里面，找到 **“settings”** 仓库设置并点击。
2. 然后，在仓库设置里拉到最底部，找到 **“Danger Zone”** 危险区域。
3. 点击 **“Delete this repository”** 这样就可以删除该仓库了。

## 二、删除某个文件

我们知道，在 Github 上我们只能删除仓库，并不能删除文件夹，所以只能采用命令的形式来解决该问题。
