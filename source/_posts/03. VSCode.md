...
mathjax: false
password:
summary: VSCode 作为前端主流的开发利器之一，本文整理了一些自用的配置和插件方便日常开发。
tags:
- VSCode
categories:
- 前端
...
退而求其次，即便你不属于以上任何范畴，你还可以把 VSCode 当作最简单的**文本编辑器**来使用。
...
大部分工具，掌握 20% 的技能，可以应对 80% 的工作。但我想说的是：那从来都不是同样的 20%，每个人都会用到不同的功能，掌握下面这些**高频核心快捷键**，足矣。
...
举个例子，我们在编辑器中输入缩写代码：`ul>li*4` ，然后按下 Tab 键，即可得到如下代码片段：
...
这个插件可以直接在 VSCode 中练习算法并提交到官网账号下，美中不足的是不能像在官网一样看到题解答案。

### 5.1 GitLens 【荐】

强烈建议安装`GitLens`，在协作开发中，它是最不可或缺的一个插件。
...
另外，还有个`Rainbow Brackets`插件，也可突出显示成对的括号。

### 5.6 sftp：文件传输 【荐】

如果你需要将本地文件通过 ftp 的形式上传到局域网服务器，可以安装`sftp`这个插件。
...
VSCode 支持将配置云同步，当我们换个电脑时，即可将日常配置登录 GitHub 后同步到本地。
...
如果你远程的配置没有成功同步到本地，那可能是网络的问题需要用 Clash 或 v2rayN 等 VPN，此时，可以使用快捷键 「Command + Shift + P」，在弹出的命令框中输入 sync，并选择「下载配置」，多试几次。
